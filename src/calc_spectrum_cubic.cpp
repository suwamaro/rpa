...
void calc_spectrum_cubic(double U, int L, double eta){
  ...
  auto output_spectrum = [&](){
    ...
    for(int o=0; o < n_omegas; o++){
      ...
      double factor_dsf = 2.;
      cx_double chi_xy = calc_intensity_cubic( L, *ts, mu, U, delta, qx, qy, qz, cx_omega, false );
      ...
      cx_double chi_z = calc_intensity_cubic( L, *ts, mu, U, delta, qx, qy, qz, cx_omega, true );
